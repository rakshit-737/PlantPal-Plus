...
var it=Object.defineProperty;var o=(e,t)=>it(e,"name",{value:t,configurable:!0});var cs=(e,t)=>()=>(e&&(t=e(e=0)),t);var ds=(e,t)=>{for(var r in t)it(e,r,{get:t[r],enumerable:!0})};var _t={};ds(_t,{getPool:()=>_,initPool:()=>Pe,setPoolForTesting:()=>Es,transaction:()=>R});import hs from"npm:pg@8.13.1";function Pe(e,t=10,r){return j=new hs.Pool({connectionString:e,max:t,idleTimeoutMillis:3e4,
...
throw r}finally{t.release()}}function Es(e){j=e}var j,k=cs(()=>{"use strict";o(Pe,"initPool");o(_,"getPool");o(R,"transaction");o(Es,"setPoolForTesting")});import{createHmac as ai}from"node:crypto";import ui from"node:process";import li from"npm:express@4.21.2";import jo from"npm:cors@2.8.5";import Bo from"npm:cookie-parser@1.4.7";import jr from"npm:express@4.21.2";import Yo from"npm:helmet@8.0.0";import{Router as xs}from"npm:express@4.21.2";import{createHash as As}from"node:crypto";import{z as S}from"npm:zod@3.24.1";var _s=S.object({NODE_ENV:S.enum(["development","test","production"]).default("development"),PORT:S.coerce.number().int().min(1).max(65535).default(3e3),DATABASE_URL:S.string().url().describe("Postgre\
...
or",this.code=t,this.status=oe[t].status,this.messageKey=oe[t].messageKey,this.details=n?.details,this.context=n?.context}},x=o((e,t)=>new m("VALIDATION_FAILED",e,t?{details:t}:void 0),"badRequest");var N=o((e="That resource could not be found.")=>new m("NOT_FOUND",e),"notFound");import ms from"npm:pino@9.5.0";var h=ms({level:process.env.LOG_LEVEL??"info",redact:{paths:["password","passwordHash","password_hash","*.password","*.passwordHash","*.password_hash","refreshToken","refresh_toke\
n","*.refreshToken","*.refresh_token","authorization","req.headers.authorization","req.headers.cookie"],censor:"[redacted]"},base:{service:"plantpal-api"}});import{createHash as ps,randomBytes as gs,timingSafeEqual as Ri}from"node:crypto";import $e from"npm:jsonwebtoken@9.0.2";var fs=900,ys=720*60*60,ws=32,ut=10,lt="plantpal-api",ct="plantpal-clients";function Le(e,t,r,n=1,s=Math.floor(Date.now()/1e3)){let a={sub:e,sid:t,ver:n,jti:crypto.randomUUID(),iss:lt,aud:ct,iat:s,exp:s+
fs};return $e.sign(a,r,{algorithm:"HS256"})}o(Le,"signAccessToken");function dt(e,t){try{let r=$e.verify(e,t,{algorithms:["HS256"]});return r.iss!==void 0&&r.iss!==lt||r.aud!==void 0&&r.aud!==ct?{ok:!1,
reason:"invalid"}:{ok:!0,claims:r}}catch(r){return r instanceof $e.TokenExpiredError?{ok:!1,reason:"expired"}:{ok:!1,reason:"invalid"}}}o(dt,"verifyAccessToken");function ie(){let e=gs(ws).toString("b\
ase64url");return{token:e,digest:H(e)}}o(ie,"issueRefreshToken");function H(e){return ps("sha256").update(e,"utf8").digest("hex")}o(H,"digestRefreshToken");function ae(e=new Date){return new Date(e.getTime()+ys*1e3)}o(ae,"refreshTokenExpiresAt");k();async function mt(e){return await R(async t=>{let{rows:[r]}=await t.query(`insert into users (email, email_normalised, password_hash, minimum_age_confirmed)
         ...
         from auth_tokens where id = $1`,[e]);if(d?.in_grace){let{rows:f}=await a.query(`update auth_tokens set consumed_at = now()
           ...
           returning id`,[e]);if(f.length>0){let y=ae(),{token:b}=ie(),T=H(b);return await a.query(`insert into auth_tokens
             ...
             values ($1, $2, $3, $4,
                     ...
                     $5, $6,
                     (select family_created_at from auth_tokens where id = $4))`,[n,r,t,e,T,y]),{kind:"ok",session:{sessionId:r,tokenFamilyId:t,refreshToken:b,refreshTokenDigest:T}}}}return await Rs(a,
...
ndRotateToken");async function Rs(e,t,r,n){await e.query(`update auth_sessions
     ...
     where token_family_id = $1 and consumed_at is null`,[t])}o(Rs,"revokeTokenFamily");var At="$argon2id$v=19$m=19456,t=2,p=1$AAAAAAAAAAAAAAAAAAAAAA$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";import bt from"npm:bcryptjs@2.4.3";var Tt=12,kt=128,B=Object.freeze({memoryCost:19456,timeCost:2,parallelism:1,outputLen:32,saltLength:16}),xt=12,J;async function It(){if(J!==void 0)return J;try{J=await import("npm:@node-rs/argon2@2.0.2"),h.info(
...
ifyPassword");function Nt(e){return e.trim().toLowerCase()}o(Nt,"normaliseEmail");function Ts(e){let t=e.ip??"0.0.0.0";return t.includes(":")?t.split(":").slice(0,3).join(":")+"::":t.split(".").slice(0,3).join(".")+
".0"}o(Ts,"ipPrefix");function ks(e){let t=e.header("x-plantpal-device");return t&&t.replace(/[\x00-\x1f]/g,"").replace(/\s+/g," ").trim().slice(0,120)||null}o(ks,"deviceLabel");function Fe(e){let t=e.
header("x-plantpal-client");return t==="IOS"||t==="ANDROID"||t==="WEB"?t:"WEB"}o(Fe,"platform");function He(){return F().JWT_ACCESS_SECRET}o(He,"accessSecret");var bs=250;async function vt(e,t=bs){let r=t-
...
a);try{await mt({email:s,passwordHash:c,confirmedAge:i})}catch(d){if(!(d&&typeof d=="object"&&"__appError"in d))throw d}h.info({email_digest:As("sha256").update(l).digest("hex").slice(0,16)},"registra\
...
let i=Nt(s),u=Ts(e),l=await yt(i);if(l.failures>=5){let q=(l.lastFailureAt?.getTime()??Date.now())+l.lockSeconds*1e3;if(q>Date.now()){let X=Math.ceil((q-Date.now())/1e3);throw await C(i,u,"LOCKED_OUT"),
...
e),installationId:b,deviceLabel:ks(e),ipAddressHash:u,userAgent:(e.get("user-agent")??"").slice(0,200)});await ft(c.id),await C(i,u,"SUCCESS");let M={access_token:Le(c.id,T.sessionId,He(),c.token_version),
...
leware")}o(Mt,"rateLimit");var V=xs();process.env.NODE_ENV!=="test"&&V.use(Mt({windowMs:6e4,max:30}));V.post("/register",Ct);V.post("/login",$t);V.post("/refresh",Lt);V.post("/logout",Pt);V.get("/me",E,Ut);var qt=V;import{Router as zs}from"npm:express@4.21.2";import{z as W}from"npm:zod@3.24.1";function g(e){let t=e.userId;if(typeof t!="string"||t.length===0)throw new m("AUTHENTICATION_REQUIRED","Authentication is required.");return t}o(g,"getUserId");function A(...e){return Object.freeze(Object.fromEntries(e.map(t=>[t,t])))}o(A,"asEnum");var Ve=A("NORTHERN","SOUTHERN","EQUATORIAL"),w=A("SPRING","SUMMER","AUTUMN","WINTER","YEAR_ROUND"),ea=A("METRIC",
...
DARD_POTTING","PEAT_BASED","COCO_COIR","SEMI_HYDRO_LECA","OTHER"),ce=A("INDOOR","OUTDOOR"),ee=A("NONE","HEATED_DRY_WINTER","AIR_CONDITIONED","HUMID_ROOM"),ta=A("THRIVING","NEEDS_ATTENTION","CRITICAL",
"DORMANT"),na=A("WALK","RUN","CYCLE","SWIM","STRENGTH","YOGA","HIIT","SPORT","OTHER"),ra=A("LOW","MODERATE","VIGOROUS"),sa=A("HEAVIEST_WEIGHT","BEST_ESTIMATED_1RM","BEST_REP_COUNT"),oa=A("BREAKFAST","\
LUNCH","DINNER","SNACK"),de=A("MALE","FEMALE","PREFER_NOT_TO_SAY"),Y=A("SEDENTARY","LIGHTLY_ACTIVE","MODERATELY_ACTIVE","VERY_ACTIVE","EXTRA_ACTIVE"),ia=A("LOSE","MAINTAIN","GAIN"),aa=A("GRAM","MILLIL\
ITRE","PIECE","CUP","TABLESPOON","SLICE","CUSTOM"),ua=A("SYNCED","PENDING","SYNCING","FAILED"),la=A("PENDING","SENT","DELIVERED","FAILED","SUPPRESSED","CANCELLED");function We(e){if(!Number.isFinite(e))throw new RangeError(`roundHalfUp expected a finite number, received ${e}`);return Math.sign(e)*Math.floor(Math.abs(e)+.5)}o(We,"roundHalfUp");function K(e,t){if(!Number.
...
e,t),r)}o(Ft,"clamp");var Is=[w.WINTER,w.WINTER,w.SPRING,w.SPRING,w.SPRING,w.SUMMER,w.SUMMER,w.SUMMER,w.AUTUMN,w.AUTUMN,w.AUTUMN,w.WINTER],Ds=Object.freeze({[w.WINTER]:w.SUMMER,[w.SUMMER]:w.WINTER,[w.SPRING]:w.AUTUMN,[w.AUTUMN]:w.
SPRING,[w.YEAR_ROUND]:w.YEAR_ROUND});function Ns(e,t){if(t===Ve.EQUATORIAL)return w.YEAR_ROUND;let r=Is[e-1];if(r===void 0)throw new RangeError(`seasonForMonth expected a month in 1..12, received ${e}`);
return t===Ve.NORTHERN?r:Ds[r]}o(Ns,"seasonForMonth");function Ht(e,t){let r=/^(\d{4})-(\d{2})-(\d{2})$/.exec(e);if(!r?.[2])throw new RangeError(`seasonForLocalDate expected a YYYY-MM-DD date, receive\
d "${e}"`);let n=Number(r[2]);if(n<1||n>12)throw new RangeError(`seasonForLocalDate received an out-of-range month in "${e}"`);return Ns(n,t)}o(Ht,"seasonForLocalDate");var vs=Object.freeze({[w.SPRING]:.95,[w.SUMMER]:.8,[w.AUTUMN]:1.15,[w.WINTER]:1.4,[w.YEAR_ROUND]:1}),Ss=Object.freeze({[Z.LOW]:1.25,[Z.MEDIUM]:1.1,[Z.BRIGHT_INDIRECT]:1,[Z.DIRECT_SUN]:.85}),Os=Object.
freeze({[$.FABRIC]:.75,[$.TERRACOTTA]:.8,[$.CONCRETE]:.9,[$.CERAMIC_GLAZED]:1,[$.OTHER]:1,[$.METAL]:1.05,[$.PLASTIC]:1.1}),Cs=Object.freeze({[O.ORCHID_BARK]:.75,[O.CACTUS_SUCCULENT]:.85,[O.GARDEN_SOIL]:.95,
[O.STANDARD_POTTING]:1,[O.OTHER]:1,[O.PEAT_BASED]:1.1,[O.COCO_COIR]:1.1,[O.SEMI_HYDRO_LECA]:1.3}),$s=Object.freeze({[ce.INDOOR]:1,[ce.OUTDOOR]:.85}),Ls=Object.freeze({[ee.HEATED_DRY_WINTER]:.85,[ee.AIR_CONDITIONED]:.9,
[ee.NONE]:1,[ee.HUMID_ROOM]:1.2});function Ps(e){if(e==null)return 1;if(!Number.isFinite(e)||e<=0)throw new RangeError(`potDiameterFactor expected a positive diameter, received ${e}`);return e<10?.8:e<
15?.9:e<20?1:e<30?1.15:e<40?1.3:1.45}o(Ps,"potDiameterFactor");function Us(e){return e===!1?1.15:1}o(Us,"drainageFactor");function Vt(e){let{baseIntervalDays:t,minIntervalDays:r,maxIntervalDays:n,season:s,
...
exceeds max ${n}`);let u=vs[s],l=Ss[a],c=e.potMaterial?Os[e.potMaterial]:1,d=Ps(e.potDiameterCm),f=Us(e.hasDrainage),y=c*d*f,b=$s[i],T=e.soilType?Cs[e.soilType]:1,D=i===ce.OUTDOOR?1:e.indoorClimate?Ls[e.
...
fDiameter:d,fDrainage:f,fEnv:M,fPlacement:b,fSoil:T,fClimate:D,rawInterval:Se,effectiveIntervalDays:ls,clamped:Oe}}o(Vt,"computeWateringInterval");var Ta=Object.freeze({protein:4,carbohydrate:4,fat:9});var ka=Object.freeze({[de.MALE]:5,[de.FEMALE]:-161,[de.PREFER_NOT_TO_SAY]:-78}),ba=Object.freeze({[Y.SEDENTARY]:1.2,[Y.LIGHTLY_ACTIVE]:1.375,[Y.MODERATELY_ACTIVE]:1.55,[Y.VERY_ACTIVE]:1.725,[Y.EXTRA_ACTIVE]:1.9}),
xa=Object.freeze({bodyMassKg:{min:30,max:400},heightCm:{min:100,max:250},ageYears:{min:16,max:120}});function _e(e,t,r){if(!Number.isFinite(e)||e<1||e>23)throw new RangeError(`metValue must be between 1.0 and 23.0, received ${e}`);if(!Number.isFinite(t)||t<=0)throw new RangeError(`bodyMassKg must be \
...
be non-negative, received ${t}`);return K(e*t,1)}o(ge,"setVolumeKg");function fe(e){let t=e.reduce((r,n)=>r+n.reps*n.weightKg,0);return K(t,1)}o(fe,"totalVolumeKg");var Ge=/^\d{4}-\d{2}-\d{2}$/;function Ms(e,t){if(!Ge.test(e)||!Ge.test(t))throw new RangeError("local dates must be YYYY-MM-DD");return Math.round((Date.parse(t)-Date.parse(e))/864e5)}o(Ms,"localDateD\
...
let r=Ms(e.lastCountedDate,t);if(r<=0)return e;if(r===1){let s=e.currentLength+1;return{...e,currentLength:s,longestLength:Math.max(e.longestLength,s),lastCountedDate:t}}let n=r-1;if(n<=e.freezeTokens){
...
1),lastCountedDate:t}}o(Gt,"advanceStreakOnLog");k();function qs(e){return{currentLength:e?.current_length??0,longestLength:e?.longest_length??0,lastCountedDate:e?.last_counted_date??null,freezeTokens:e?.freeze_tokens??0}}o(qs,"toState");async function jt(e,t,r,n){
await e.query(`insert into streaks (user_id, streak_type)
     ...
     for update`,[t,r]),a=Gt(qs(s[0]),n);return await e.query(`update streaks
...
streak_type = 'NUTRITION'",overall_streak:"select coalesce(max(current_length), 0)::int as v from streaks where user_id = $1 and streak_type = 'OVERALL'",all_modules_in_day:`select case when exists (
                       ...
                       ) then 1 else 0 end as v`};function Fs(e,t){return e.filter(r=>{let n=t.get(r.metric);return n!==void 0&&n>=r.gte})}o(Fs,"evaluateUnlocks");async function Yt(e,t){let{rows:r}=await e.
...
let s=new Map;for(let i of new Set(n.map(u=>u.metric))){let u=Bt[i];if(!u)continue;let{rows:l}=await e.query(u,[t]);s.set(i,Number(l[0]?.v??0))}let a=Fs(n,s);for(let i of a)await e.query(`insert into \
user_achievements (user_id, achievement_id, unlocked_at, progress_pct)
       values ($1, $2, now(), 100)
       on conflict (user_id, achievement_id)
       do update set unlocked_at = coalesce(user_achievements.unlocked_at, now()), progress_pct = 100`,[t,i.id]);return a.map(i=>i.code)}o(Yt,"evaluateAchievements");var Hs={PLANT_CARE:`select (not ex\
ists (
              ...
              from meals
              where user_id = $1 and local_date_str = $2 and deleted_at is null`};async function Vs(e,t,r){return R(async n=>{let{rows:s}=await n.query(Hs[t],[e,r]),a=s[0]?.met===!0;if(a){await jt(n,e,
t,r);let{rows:u}=await n.query(`select streak_type, last_counted_date from streaks
         ...
         for update`,[e]);u.length===3&&u.every(c=>c.last_counted_date===r)&&await jt(n,e,"OVERALL",r)}let i=await Yt(n,e);return{met:a,unlocked:i}})}o(Vs,"recordDailyLog");async function ye(e){try{let t=await R(
...
async function L(e,t,r){try{let{met:n,unlocked:s}=await Vs(e,t,r);s.length>0&&h.info({userId:e,scope:t,met:n,unlocked:s},"achievements unlocked")}catch(n){h.warn({err:n,userId:e,scope:t},"engagement u\
pdate failed (log write unaffected)")}}o(L,"recordDailyLogSafe");k();async function zt(e){let t=_(),{rows:r}=await t.query(`select p.id as plant_id, p.user_id, p.nickname, p.next_water_due_at
     ...
     where p.deleted_at is null
       ...
       do nothing`,[n.user_id,n.reminder_type,n.target_entity_id,n.target_entity_type,n.title,n.body,n.due_at_utc]);r+=s.rowCount??0}return r}o(Qt,"insertReminders");var Ws=["OFF","WINDOW","SCHEDULED_\
...
query(`select user_id, timezone, quiet_hours_mode, quiet_start_time, quiet_end_time,
     ...
     from user_settings
     where user_id = any ($1::uuid[])`,[e]);for(let s of n)t.set(s.user_id,{timezone:s.timezone,quiet_hours_mode:Ws.includes(s.quiet_hours_mode)?s.quiet_hours_mode:"OFF",quiet_start_time:Kt(s.quiet_start_time),
...
null,t.base_interval_days,t.min_interval_days,t.max_interval_days,t.photo_url??null]);return n}o(un,"createPlant");var Gs=new Set(["nickname","species_id","room","acquisition_date","light_exposure","p\
...
([l,c])=>c!==void 0&&Gs.has(l));if(n.length===0)return he(e,t);let s=n.map(([l],c)=>`${l}=$${c+3}`).join(", "),a=n.map(([,l])=>l),i=_(),{rows:u}=await i.query(`UPDATE plants SET ${s}, updated_at=now()\
...
_name, base_interval_days, min_interval_days,
            max_interval_days, default_light, default_soil, care_notes, image_url
     FROM species WHERE NOT is_custom ORDER BY common_name LIMIT 200`);return r}o(fn,"listSpecies");var yn=["WATER","FERTILIZE","PRUNE","REPOT","MIST","ROTATE","TREAT"],js=W.string().trim().max(2048).url().refine(e=>/^https?:\/\//i.test(e),"photo_url must be an http(s) URL"),Bs=W.object({photo_url:js,
...
strict(),Ys=20;function Ks(e){return e.issues.slice(0,Ys).map(t=>({field:t.path.join(".")||"(root)",issue:t.message}))}o(Ks,"detailsFor");function Re(e,t){let r=W.string().uuid().safeParse(e);if(!r.success)
...
try{let n=g(e),s=Re(e.params.id,"id"),a=Bs.safeParse(e.body);if(!a.success)throw x("The request failed validation.",Ks(a.error));let i=a.data,u=await mn(n,s,{photo_url:i.photo_url,photo_storage_key:i.
...
r(n)}}o(In,"removeGrowthEntry");async function Dn(e,t,r){try{let n=await fn(e.query.q);t.json(n)}catch(n){r(n)}}o(Dn,"searchSpecies");var I=zs();I.use(E);I.get("/species",Dn);I.get("/",wn);I.post("/",En);I.get("/:id",hn);I.put("/:id",Rn);I.delete("/:id",An);I.post("/:id/care",Tn);I.get("/:id/care",kn);I.post("/:id/growth",bn);I.get(
"/:id/growth",xn);I.delete("/:id/growth/:entryId",In);var Nn=I;import{Router as Xs}from"npm:express@4.21.2";k();async function vn(e){if(e.length===0)return new Map;let t=_(),{rows:r}=await t.query(`select workout_id, id, set_index, reps,
...
getWorkoutHandler");var Qs=new Set(["WALK","RUN","CYCLE","SWIM","STRENGTH","YOGA","HIIT","SPORT","OTHER"]);async function Mn(e,t,r){try{let n=e.body;if(!n.activity_type||!Qs.has(n.activity_type))throw x(
...
n);t.json({exercises:s})}catch(n){r(n)}}o(Fn,"searchExercises");async function Hn(e,t,r){try{let n=await Ln(z(e));t.json({personal_records:n})}catch(n){r(n)}}o(Hn,"getPersonalRecordsHandler");var P=Xs();P.use(E);P.get("/exercises",Fn);P.get("/personal-records",Hn);P.get("/summary",qn);P.get("/",Pn);P.post("/",Mn);P.get("/:id",Un);var Vn=P;import{Router as no}from"npm:express@4.21.2";import{z as v}from"npm:zod@3.24.1";k();var Gn=`id, name, brand,
...
query(`update foods
      ...
      where id = $1 and created_by = $2 and is_custom and deleted_at is null`,[e,t]);return(n??0)>0}o(Yn,"softDeleteCustomFood");var Js=2e3;async function Kn(e,t){let r=_(),{rows:n}=await r.query(`sel\
ect id, meal_type,
...
carbs_g:0,fat_g:0});return{meals:s,water_ml_total:Number(a?.water_ml_total??0),water_goal_ml:a?.water_goal_ml??Js,totals:i}}o(Kn,"getDailySummary");async function Te(e,t){return R(async r=>{let n=t.items.
...
ogWater");var zn=["BREAKFAST","LUNCH","DINNER","SNACK"],Ye=["GRAM","MILLILITRE","PIECE","CUP","TABLESPOON","SLICE","CUSTOM"];function Zs(){return new Date().toISOString().slice(0,10)}o(Zs,"todayUtcDateStr");function Ke(e){
return typeof e=="string"&&/^\d{4}-\d{2}-\d{2}$/.test(e)}o(Ke,"isValidDateStr");function eo(e,t){let r=v.string().uuid().safeParse(e);if(!r.success)throw new m("VALIDATION_FAILED",`${t} must be a UUID\
.`,{details:[{field:t,issue:"invalid"}]});return r.data}o(eo,"requireUuidParam");async function Qn(e,t,r){try{let n=e.query.q;if(n!==void 0&&typeof n!="string")throw new m("VALIDATION_FAILED","Query p\
arameter q must be a string.");let s=g(e),a=await jn((n??"").trim(),s);t.status(200).json({foods:a})}catch(n){r(n)}}o(Qn,"searchFoodsHandler");var to=v.object({name:v.string().trim().min(1).max(120),brand:v.
...
optional()}).strict();async function Xn(e,t,r){try{let n=g(e),s=to.safeParse(e.body);if(!s.success)throw new m("VALIDATION_FAILED","The request failed validation.",{details:s.error.issues.slice(0,20).
...
deleted:i.deleted,retention_days:te}]});t.status(201).json(i.food)}catch(n){r(n)}}o(Xn,"createCustomFoodHandler");async function Jn(e,t,r){try{let n=g(e),s=eo(e.params.id,"id");if(!await Yn(s,n))throw N();
t.json({status:"deleted"})}catch(n){r(n)}}o(Jn,"deleteCustomFoodHandler");async function Zn(e,t,r){try{let n=e.query.date??Zs();if(!Ke(n))throw new m("VALIDATION_FAILED","date must be YYYY-MM-DD.");let s=g(
...
void 0});await ye(a),t.status(201).json(i)}catch(n){r(n)}}o(tr,"logWaterHandler");var U=no();U.use(E);U.get("/foods/search",Qn);U.post("/foods",Xn);U.delete("/foods/:id",Jn);U.get("/summary",Zn);U.post("/meals",er);U.post("/water",tr);var nr=U;import{Router as oo}from"npm:express@4.21.2";k();var ro=1e4,rr=2e3;async function sr(e,t){let r=_(),[n,s,a,i,u]=await Promise.all([r.query(`select current_length, longest_length
         from streaks
...
overdue:Number(a.rows[0]?.count??0)},fitness:{steps:c,goal:ro},nutrition:{calories_consumed:d,target:rr},today_list:f}}o(sr,"getDashboard");function so(){return new Date().toISOString().slice(0,10)}o(so,"todayUtcDateStr");async function or(e,t,r){try{let n=typeof e.query.date=="string"&&/^\d{4}-\d{2}-\d{2}$/.test(e.query.date)?e.query.date:
so(),s=g(e),a=await sr(s,n);t.status(200).json(a)}catch(n){r(n)}}o(or,"getDashboardHandler");var ze=oo();ze.use(E);ze.get("/",or);var ir=ze;import{Router as io}from"npm:express@4.21.2";k();async function ar(e){let t=_(),{rows:r}=await t.query(`select a.id as a_id, a.code, a.name, a.description, a.module, a.icon,
...
r(n)}});var pr=be;import{Router as lo}from"npm:express@4.21.2";import{z as G}from"npm:zod@3.24.1";k();var uo=5;async function fr(e,t){try{return await gr(e,t)}catch(r){if(typeof r=="object"&&r!==null&&r.code==="23505")return gr(e,t);throw r}}o(fr,"registerToken");async function gr(e,t){return R(async r=>{
let{rows:n}=await r.query(`select id, user_id from device_push_tokens
       ...
       where id in (
         ...
         offset $2
       )`,[e,uo]);let{rows:i}=await r.query(`select id, platform, device_label, app_version, permission_status, last_confirmed_at
...
if(e.length===0)return;await _().query(`update device_push_tokens
     ...
     where token = any ($1::text[]) and status = 'ACTIVE'`,[e,t])}o(wr,"revokeTokens");var co=G.object({expo_push_token:G.string().min(20).max(200).regex(/^Expo(nent)?PushToken\[.+\]$/),platform:G.enum(["IOS","ANDROID"]),client_installation_id:G.string().uuid(),device_label:G.string().max(
64).optional(),app_version:G.string().max(20).optional(),permission_status:G.enum(["GRANTED","DENIED","UNDETERMINED"])}).strict(),Qe=lo();Qe.use(E);Qe.post("/",async(e,t,r)=>{try{let n=co.safeParse(e.
...
g(e),{...n.data,device_label:n.data.device_label?.trim()||void 0});t.status(200).json({id:s,devices:a})}catch(n){r(n)}});var hr=Qe;import{Router as bo}from"npm:express@4.21.2";import{z as p}from"npm:zod@3.24.1";k();async function Er(e,t,r,n){let s=_(),{rows:a}=await s.query(`insert into sync_events (user_id, client_idempotency_key, entity_type, payload)
     values ($1, $2, $3, $4)
...
$2`,[t,r]);return s[0]?.id??null}o(Je,"findEntityIdByKey");var _o=50,xe=p.string().regex(/^\d{4}-\d{2}-\d{2}$/).refine(e=>{let t=Date.parse(e);return t>Date.now()-366*864e5&&t<Date.now()+2*864e5},"local_date_str outside the accepted window"),mo=p.object({plant_id:p.
string().uuid(),action_type:p.enum(["WATER","FERTILIZE","PRUNE","REPOT","MIST","ROTATE","TREAT"]),note:p.string().max(500).optional(),local_date_str:xe}).strict(),po=p.object({set_index:p.number().int().
min(1).max(200).optional(),reps:p.number().int().min(0).max(1e3),weight_kg:p.number().min(0).max(1e3)}).strict(),go=p.object({activity_type:p.string().min(1).max(40),duration_mins:p.number().int().min(
...
steps:p.number().int().min(0).max(2e5).optional(),note:p.string().max(500).optional(),local_date_str:xe,sets:p.array(po).max(200).optional()}).strict(),fo=p.object({food_id:p.string().uuid().optional(),
...
number().min(0).max(1e4),carbs_g:p.number().min(0).max(1e4),fat_g:p.number().min(0).max(1e4)}).strict(),yo=p.object({meal_type:p.enum(["BREAKFAST","LUNCH","DINNER","SNACK"]),note:p.string().max(500).optional(),
local_date_str:xe,items:p.array(fo).min(1).max(50)}).strict(),wo=p.object({amount_ml:p.number().int().min(1).max(5e3),goal_ml_at_log:p.number().int().min(1).max(2e4).optional(),local_date_str:xe}).strict(),
ho=p.object({client_idempotency_key:p.string().regex(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i),entity_type:p.enum(["PLANT_CARE_EVENT","WORKOUT","MEAL","WATER_LOG"]),payload:p.
unknown()}).strict(),Eo=p.object({events:p.array(ho).min(1).max(_o)}).strict();function Ro(e){return typeof e=="object"&&e!==null&&e.code==="23505"}o(Ro,"isUniqueViolation");var Ao={PLANT_CARE_EVENT:"\
plant_care_events",WORKOUT:"workouts",MEAL:"meals",WATER_LOG:"water_logs"},To={PLANT_CARE_EVENT:"PLANT_CARE",WORKOUT:"FITNESS",MEAL:"NUTRITION",WATER_LOG:null};async function ko(e,t,r,n){switch(r){case"\
PLANT_CARE_EVENT":{let s=mo.parse(n);return await Ee(e,s.plant_id,s.action_type,s.note,s.local_date_str,t),Je("plant_care_events",e,t)}case"WORKOUT":{let s=go.parse(n),a=(s.sets??[]).map((c,d)=>{let f=ge(
...
total_volume_kg:i,steps:s.steps,note:s.note,local_date_str:s.local_date_str,client_idempotency_key:t,sets:a.length>0?a:void 0})).id}case"MEAL":{let s=yo.parse(n);return(await Te(e,{...s,client_idempotency_key:t})).
id}case"WATER_LOG":{let s=wo.parse(n);return(await ke(e,{...s,client_idempotency_key:t})).id}}}o(ko,"applyEvent");async function Ar(e,t,r){try{let n=g(e),s=Eo.safeParse(e.body);if(!s.success)throw new m(
...
result_entity_id,error_code:l.error_code});continue}try{let d=await ko(n,u,i.entity_type,i.payload);await Xe(l.id,d);let f=To[i.entity_type],y=i.payload.local_date_str;f&&y?await L(n,f,y):i.entity_type===
"WATER_LOG"&&await ye(n),a.push({client_idempotency_key:u,status:"PROCESSED",replay:!1,entity_id:d,error_code:null})}catch(d){if(Ro(d)){let D=await Je(Ao[i.entity_type],n,u);if(D){await Xe(l.id,D),a.push(
...
FAILED",replay:!1,entity_id:null,error_code:b})}}t.status(200).json({results:a})}catch(n){r(n)}}o(Ar,"drainOutboxHandler");var Ze=bo();Ze.use(E);Ze.post("/outbox",Ar);var Tr=Ze;import{Router as Oo}from"npm:express@4.21.2";import{z as Io}from"npm:zod@3.24.1";k();var br=`timezone, hemisphere, locale, unit_system, theme, week_start_day,
...
_settings (user_id) values ($1) on conflict (user_id) do nothing",[e]);let{rows:r}=await t.query(`select ${br} from user_settings where user_id = $1`,[e]);return xr(r[0])}o(Ie,"getSettings");var xo=new Set(
...
cation_cap","reduce_motion","larger_text","high_contrast","analytics_opt_in"]);async function Ir(e,t){let r=Object.entries(t).filter(([u,l])=>l!==void 0&&xo.has(u));if(r.length===0)return Ie(e);let n=_();
...
user_settings set ${s}, updated_at=now()
     where user_id=$1
     returning ${br}`,[e,...a]);return xr(i[0])}o(Ir,"updateSettings");var Do={hemisphere:["NORTHERN","SOUTHERN","EQUATORIAL"],unit_system:["METRIC","IMPERIAL"],theme:["LIGHT","DARK","SYSTEM"],week_start_day:["SUNDAY","MONDAY"],quiet_hours_mode:["OFF","WINDOW","SCHEDULED\
_ONLY"]},No=["plant_care_enabled","fitness_enabled","nutrition_enabled","reduce_motion","larger_text","high_contrast","analytics_opt_in"],vo=Io.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).nullable(),Dr=[
"quiet_start_time","quiet_end_time"],So=["quiet_hours_mode",...Dr];async function Nr(e,t,r){try{t.json(await Ie(g(e)))}catch(n){r(n)}}o(Nr,"getSettingsHandler");async function vr(e,t,r){try{let n=e.body??
{},s=[],a={};for(let[c,d]of Object.entries(Do)){let f=n[c];f!==void 0&&(typeof f!="string"||!d.includes(f)?s.push({field:c,issue:`must_be_one_of:${d.join(",")}`}):a[c]=f)}for(let c of No){let d=n[c];d!==
void 0&&(typeof d!="boolean"?s.push({field:c,issue:"must_be_boolean"}):a[c]=d)}for(let c of Dr){let d=n[c];if(d===void 0)continue;let f=vo.safeParse(d);f.success?a[c]=f.data:s.push({field:c,issue:"mus\
...
led.",{details:[{field:"modules",issue:"at_least_one_module_required"}]});if(So.some(c=>c in a)&&l.quiet_hours_mode==="WINDOW"){if(l.quiet_start_time===null||l.quiet_end_time===null)throw new m("VALID\
...
ATION_FAILED","Quiet hours need a different start and end time.",{details:[{field:"quiet_end_time",issue:"window_start_equals_end"}]})}t.json(await Ir(i,a))}catch(n){r(n)}}o(vr,"updateSettingsHandler");var De=Oo();De.use(E);De.get("/",Nr);De.put("/",vr);var Sr=De;import{Router as Uo}from"npm:express@4.21.2";import{z as et}from"npm:zod@3.24.1";k();var Co=30,Or="PENDING_DELETION",re="status, deletion_requested_at, purge_after";async function Cr(e){let t=_(),{rows:r}=await t.query(`select ${re} from users where id = $1`,[e]);return r[0]??null}o(Cr,
...
already_pending",state:n};let{rows:[s]}=await r.query(`update users
       ...
       where id = $1
       returning ${re}`,[e,Co]);return await r.query(`update auth_sessions
 ...
 from users where id = $1 for update`,[e]);if(!r)return{kind:"missing"};if(r.status!==Or)return{kind:"not_pending",state:r};let{rows:[n]}=await t.query(`update users
       ...
       returning ${re}`,[e]);return{kind:"cancelled",state:n}})}o(Lr,"cancelDeletion");var $o=et.object({password:et.string().min(1,"Your password is required to confirm deletion.")}).strict();function Lo(e){return new m("VALIDATION_FAILED","The request failed validation.",{details:e.issues.
slice(0,10).map(t=>({field:t.path.join(".")||"(root)",issue:t.message}))})}o(Lo,"validationError");function Po(e){let t=e.sessionId,r=et.string().uuid().safeParse(t);return r.success?r.data:null}o(Po,
...
json(tt(n))}catch(n){r(n)}}o(Pr,"getAccountHandler");async function Ur(e,t,r){try{let n=$o.safeParse(e.body??{});if(!n.success)throw Lo(n.error);let s=g(e),a=await Et(s);if(!a)throw Ne();if(a.password_hash===
...
"INVALID_CREDENTIALS","That password is not right.");let i=await $r(s,Po(e));if(i.kind==="missing")throw Ne();t.status(200).json({...tt(i.state),already_pending:i.kind==="already_pending"})}catch(n){r(
...
r deletion.");t.status(200).json(tt(n.state))}catch(n){r(n)}}o(Mr,"cancelDeletionHandler");var se=Uo();se.use(E);se.get("/",Pr);se.post("/deletion",Ur);se.delete("/deletion",Mr);var qr=se;import{ZodError as Ho}from"npm:zod@3.24.1";import{randomUUID as Mo}from"node:crypto";var Fr="x-request-id",qo=64,Fo=/^[A-Za-z0-9._-]+$/,Hr=o((e,t,r)=>{let n=e.header(Fr),a=(n&&n.length<=qo&&Fo.test(n)?n:void 0)??Mo();e.requestId=a,t.setHeader(Fr,a),r()},"requestId");function Vr(e){return e.
requestId??"unknown"}o(Vr,"getRequestId");var Vo=50;function Wo(e){return e.errors.slice(0,Vo).map(t=>({field:t.path.join(".")||"(root)",issue:t.code,message:t.message}))}o(Wo,"detailsFromZod");var Wr=o((e,t,r)=>{r(new m("NOT_FOUND",`No route\
 matches ${e.method} ${e.path}`))},"notFoundHandler");function Go(e){if(!(e instanceof Error))return!1;let t=e;return t.__appError===!0&&typeof t.code=="string"&&t.code in oe}o(Go,"isMarkedAppError");
var Gr=o((e,t,r,n)=>{let s=Vr(t),a=new Date().toISOString(),i;e instanceof m?i=e:e instanceof Ho?i=new m("VALIDATION_FAILED","The request failed validation.",{details:Wo(e)}):e instanceof SyntaxError&&
"body"in e?i=new m("MALFORMED_REQUEST","The request body is not valid JSON."):Go(e)?i=new m(e.code,e.message):i=new m("INTERNAL_ERROR","An unexpected error occurred.",{cause:e});let u={requestId:s,code:i.
...
/`)&&(n.url=n.url.slice(r.length),n.originalUrl=n.url),a()}),t.use(Hr),t.use(Yo()),t.use(jo({origin:e.corsOrigins,credentials:!0,exposedHeaders:["x-request-id"]})),t.use(jr.json({limit:e.bodyLimit??"1\
mb"})),t.use(Bo()),t.use("/api/auth",qt),t.use("/api/v1/plants",Nn),t.use("/api/v1/fitness",Vn),t.use("/api/v1/nutrition",nr),t.use("/api/v1/dashboard",ir),t.use("/api/v1/achievements",mr),t.use("/api\
...
uptime())})}),t.get("/api/v1",(n,s)=>{s.json({name:"PlantPal+ API",version:"v1"})}),t.use(Wr),t.use(Gr),t}o(Br,"createApp");k();import Jc from"npm:node-cron@4.6.0";k();import{createHmac as Ko}from"node:crypto";var nt=100,Yr=Object.freeze([{table:"profiles",column:"user_id"},{table:"user_settings",column:"user_id"},{table:"auth_sessions",column:"user_id"},{table:"auth_tokens",column:"user_id"},{table:"email_\
...
{table:"sync_events",column:"user_id"}]);async function Kr(e=nt){let{rows:t}=await _().query(`select id, email_normalised
      ...
      limit $1`,[e]);return t}o(Kr,"findAccountsDueForPurge");function zo(e,t){return Ko("sha256",t).update(e).digest("hex")}o(zo,"subjectHash");async function zr(e,t){return R(async r=>{let{rows:[n]}=await r.
query(`select id
         from users
        where id = $1
...
elect ${s}`,[e.id]),i={};Yr.forEach(({table:d},f)=>{i[d]=a?.[`t${f}`]??0});let u=zo(e.id,t),{rowCount:l}=await r.query(`update audit_events
          set user_id = null,
...
c??0,await r.query("delete from users where id = $1",[e.id]),i.users=1,await r.query(`insert into audit_events (user_id, event_type, payload)
       values (null, 'ACCOUNT_ERASED', $1::jsonb)`,[JSON.stringify({subject:u,rows:i,erased_at:new Date().toISOString()})]),{erased:!0,counts:i}})}o(zr,"purgeAccount");function Qo(){let e=F();return e.AUDIT_PEPPER??e.JWT_ACCESS_SECRET}o(Qo,"pepper");async function Qr(e=nt){let t=await Kr(e),r={due:t.length,erased:0,skipped:0,failed:0,counts:{}};if(t.length===0)return r;
let n=Qo();for(let s of t)try{let a=await zr(s,n);if(!a.erased){r.skipped++;continue}r.erased++;for(let[i,u]of Object.entries(a.counts))r.counts[i]=(r.counts[i]??0)+u}catch(a){r.failed++,h.error({err:a},
"account erasure failed; will retry on the next sweep")}return h.info(r,"account erasure sweep complete"),r}o(Qr,"runPurgePass");import cd from"npm:node-cron@4.6.0";var Xo="https://exp.host/--/api/v2/push/send",Jo=100;function Zo(e,t=Jo){let r=[];for(let n=0;n<e.length;n+=t)r.push(e.slice(n,n+t));return r}o(Zo,"chunkMessages");async function Xr(e){let t={delivered:[],
notRegistered:[],failed:[]};for(let r of Zo(e))try{let n=await fetch(Xo,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify(r)});if(!n.ok){t.failed.
...
e:s.next_water_due_at}))}o(ts,"planWateringReminders");var ei=5,ns={timezone:"UTC",quiet_hours_mode:"WINDOW",quiet_start_time:null,quiet_end_time:null,daily_notification_cap:12},Jr={hourCycle:"h23",year:"\
numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"},Zr=new Map;function ti(e){let t=Zr.get(e);if(t)return t;let r;try{r=new Intl.DateTimeFormat("en-US",{...Jr,timeZone:e})}catch{r=
new Intl.DateTimeFormat("en-US",{...Jr,timeZone:"UTC"})}return Zr.set(e,r),r}o(ti,"formatterFor");function rt(e,t){let r=ti(t).formatToParts(e),n=o(a=>r.find(i=>i.type===a)?.value??"00","part"),s=Number(
n("hour"))%24;return{dateKey:`${n("year")}-${n("month")}-${n("day")}`,minutes:s*60+Number(n("minute"))}}o(rt,"localClock");var ni=1440;function es(e){if(e===null)return null;let t=/^(\d{1,2}):(\d{2})(?::\d{2}(?:\.\d+)?)?$/.
exec(e.trim());if(!t)return null;let r=Number(t[1])*60+Number(t[2]);return r>=0&&r<ni?r:null}o(es,"wallClockMinutes");function ri(e,t){if(t.quiet_hours_mode==="OFF")return!1;if(t.quiet_hours_mode==="S\
CHEDULED_ONLY")return!0;let r=es(t.quiet_start_time),n=es(t.quiet_end_time);if(r===null||n===null||r===n)return!1;let s=rt(e,t.timezone).minutes;return r<n?s>=r&&s<n:s>=r||s<n}o(ri,"isWithinQuietHours");
function si(e,t,r){let n=rt(t,r).dateKey,s=0;for(let a of e)rt(a,r).dateKey===n&&(s+=1);return s}o(si,"sentOnLocalDay");function oi(e){let t=e.daily_notification_cap;return!Number.isFinite(t)||t<1?ns.
daily_notification_cap:Math.floor(t)}o(oi,"capOf");function rs(e,t,r={}){let n={send:[],fail:[],defer:[]},s=t.filter(i=>i.due_at_utc.getTime()<=e.getTime()).sort((i,u)=>{let l=i.due_at_utc.getTime()-u.
due_at_utc.getTime();return l!==0?l:i.id<u.id?-1:i.id>u.id?1:0}),a=new Map;for(let i of s){if(i.attempts>=ei){n.fail.push(i.id);continue}let u=r.settings?.get(i.user_id)??ns;if(ri(e,u)){n.defer.push({
id:i.id,reason:"QUIET_HOURS"});continue}let l=a.get(i.user_id);if(l===void 0){let c=r.sentAt?.get(i.user_id)??[];l=Math.max(0,oi(u)-si(c,e,u.timezone))}if(l===0){a.set(i.user_id,0),n.defer.push({id:i.
id,reason:"DAILY_CAP_REACHED"});continue}a.set(i.user_id,l-1),n.send.push(i.id)}return n}o(rs,"tick");var ss=24;async function ii(e){if(e.length===0)return 0;let t=await yr([...new Set(e.map(i=>i.user_id))]);if(t.size===0)return 0;let r=[],n=new Map;for(let i of e)for(let u of t.get(i.user_id)??[]){r.
...
RED");let a=new Set;for(let i of s.delivered)for(let u of n.get(i)??[])a.add(u);return await en([...a]),a.size}o(ii,"deliverByPush");async function os(e=new Date){let t=await zt(ss),r=ts(e,t,ss),n=await Qt(
r),s=await Zt(),a=[...new Set(s.map(f=>f.user_id))],[i,u]=await Promise.all([Xt(a),Jt(a,e)]),l=rs(e,s,{settings:i,sentAt:u});await tn(l.send),await nn(l.fail);let c=new Set(l.send),d=await ii(s.filter(
...
"No platform secret to derive from: set JWT_ACCESS_SECRET on the function explicitly.");return ai("sha256",t).update(`plantpal:${e}`).digest("hex")}o(st,"derivedSecret");function Q(e,t){let r=Deno.env.
...
Deno.env.get("SUPABASE_URL")??"https://localhost").origin,us=Ce({...ui.env,NODE_ENV:"production",DATABASE_URL:is,JWT_ACCESS_SECRET:Q("JWT_ACCESS_SECRET",st("jwt-access")),AUDIT_PEPPER:Q("AUDIT_PEPPER",
...
var ci=Q("TICK_SECRET",st("internal-tick")),di=Br({corsOrigins:us.CORS_ORIGINS,basePath:`/${ve}`}),ot=li();ot.post(`/${ve}/internal/tick`,(e,t)=>{if(e.get("authorization")!==`Bearer ${ci}`){t.status(401).
...
led"},"internal tick complete")}),t.status(202).json({status:"accepted"})});ot.use(di);h.info({slug:ve,origin:as},"PlantPal+ API starting on Supabase Edge");ot.listen(8e3);
