...
var it=Object.defineProperty;var o=(e,t)=>it(e,"name",{value:t,configurable:!0});var cs=(e,t)=>()=>(e&&(t=e(e=0)),t);var ds=(e,t)=>{for(var r in t)it(e,r,{get:t[r],enumerable:!0})};var _t={};ds(_t,{getPool:()=>_,initPool:()=>Pe,setPoolForTesting:()=>Rs,transaction:()=>R});import Es from"npm:pg@8.13.1";function Pe(e,t=10,r){return j=new Es.Pool({connectionString:e,max:t,idleTimeoutMillis:3e4,
...
throw r}finally{t.release()}}function Rs(e){j=e}var j,k=cs(()=>{"use strict";o(Pe,"initPool");o(_,"getPool");o(R,"transaction");o(Rs,"setPoolForTesting")});import{createHmac as ui}from"node:crypto";import li from"node:process";import ci from"npm:express@4.21.2";import Bo from"npm:cors@2.8.5";import Yo from"npm:cookie-parser@1.4.7";import jr from"npm:express@4.21.2";import Ko from"npm:helmet@8.0.0";import{Router as Is}from"npm:express@4.21.2";import{createHash as Ts}from"node:crypto";import{z as S}from"npm:zod@3.24.1";var _s=S.object({NODE_ENV:S.enum(["development","test","production"]).default("development"),PORT:S.coerce.number().int().min(1).max(65535).default(3e3),DATABASE_URL:S.string().url().describe("Postgre\
...
or",this.code=t,this.status=oe[t].status,this.messageKey=oe[t].messageKey,this.details=n?.details,this.context=n?.context}},x=o((e,t)=>new m("VALIDATION_FAILED",e,t?{details:t}:void 0),"badRequest");var N=o((e="That resource could not be found.")=>new m("NOT_FOUND",e),"notFound");import ms from"npm:pino@9.5.0";var ps=typeof process<"u"&&process.versions?.node?void 0:{write(e){console.log(e.endsWith(`
`)?e.slice(0,-1):e)}},h=ms({level:process.env.LOG_LEVEL??"info",redact:{paths:["password","passwordHash","password_hash","*.password","*.passwordHash","*.password_hash","refreshToken","refresh_token",
"*.refreshToken","*.refresh_token","authorization","req.headers.authorization","req.headers.cookie"],censor:"[redacted]"},base:{service:"plantpal-api"}},ps);import{createHash as gs,randomBytes as fs,timingSafeEqual as Ai}from"node:crypto";import $e from"npm:jsonwebtoken@9.0.2";var ys=900,ws=720*60*60,hs=32,ut=10,lt="plantpal-api",ct="plantpal-clients";function Le(e,t,r,n=1,s=Math.floor(Date.now()/1e3)){let a={sub:e,sid:t,ver:n,jti:crypto.randomUUID(),iss:lt,aud:ct,iat:s,exp:s+
ys};return $e.sign(a,r,{algorithm:"HS256"})}o(Le,"signAccessToken");function dt(e,t){try{let r=$e.verify(e,t,{algorithms:["HS256"]});return r.iss!==void 0&&r.iss!==lt||r.aud!==void 0&&r.aud!==ct?{ok:!1,
reason:"invalid"}:{ok:!0,claims:r}}catch(r){return r instanceof $e.TokenExpiredError?{ok:!1,reason:"expired"}:{ok:!1,reason:"invalid"}}}o(dt,"verifyAccessToken");function ie(){let e=fs(hs).toString("b\
ase64url");return{token:e,digest:H(e)}}o(ie,"issueRefreshToken");function H(e){return gs("sha256").update(e,"utf8").digest("hex")}o(H,"digestRefreshToken");function ae(e=new Date){return new Date(e.getTime()+ws*1e3)}o(ae,"refreshTokenExpiresAt");k();async function mt(e){return await R(async t=>{let{rows:[r]}=await t.query(`insert into users (email, email_normalised, password_hash, minimum_age_confirmed)
         ...
         from auth_tokens where id = $1`,[e]);if(d?.in_grace){let{rows:f}=await a.query(`update auth_tokens set consumed_at = now()
           ...
           returning id`,[e]);if(f.length>0){let y=ae(),{token:b}=ie(),T=H(b);return await a.query(`insert into auth_tokens
             ...
             values ($1, $2, $3, $4,
                     ...
                     $5, $6,
                     (select family_created_at from auth_tokens where id = $4))`,[n,r,t,e,T,y]),{kind:"ok",session:{sessionId:r,tokenFamilyId:t,refreshToken:b,refreshTokenDigest:T}}}}return await As(a,
...
ndRotateToken");async function As(e,t,r,n){await e.query(`update auth_sessions
     ...
     where token_family_id = $1 and consumed_at is null`,[t])}o(As,"revokeTokenFamily");var At="$argon2id$v=19$m=19456,t=2,p=1$AAAAAAAAAAAAAAAAAAAAAA$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";import bt from"npm:bcryptjs@2.4.3";var Tt=12,kt=128,B=Object.freeze({memoryCost:19456,timeCost:2,parallelism:1,outputLen:32,saltLength:16}),xt=12,J;async function It(){if(J!==void 0)return J;try{J=await import("npm:@node-rs/argon2@2.0.2"),h.info(
...
ifyPassword");function Nt(e){return e.trim().toLowerCase()}o(Nt,"normaliseEmail");function ks(e){let t=e.ip??"0.0.0.0";return t.includes(":")?t.split(":").slice(0,3).join(":")+"::":t.split(".").slice(0,3).join(".")+
".0"}o(ks,"ipPrefix");function bs(e){let t=e.header("x-plantpal-device");return t&&t.replace(/[\x00-\x1f]/g,"").replace(/\s+/g," ").trim().slice(0,120)||null}o(bs,"deviceLabel");function Fe(e){let t=e.
header("x-plantpal-client");return t==="IOS"||t==="ANDROID"||t==="WEB"?t:"WEB"}o(Fe,"platform");function He(){return F().JWT_ACCESS_SECRET}o(He,"accessSecret");var xs=250;async function vt(e,t=xs){let r=t-
...
a);try{await mt({email:s,passwordHash:c,confirmedAge:i})}catch(d){if(!(d&&typeof d=="object"&&"__appError"in d))throw d}h.info({email_digest:Ts("sha256").update(l).digest("hex").slice(0,16)},"registra\
...
let i=Nt(s),u=ks(e),l=await yt(i);if(l.failures>=5){let q=(l.lastFailureAt?.getTime()??Date.now())+l.lockSeconds*1e3;if(q>Date.now()){let X=Math.ceil((q-Date.now())/1e3);throw await C(i,u,"LOCKED_OUT"),
...
e),installationId:b,deviceLabel:bs(e),ipAddressHash:u,userAgent:(e.get("user-agent")??"").slice(0,200)});await ft(c.id),await C(i,u,"SUCCESS");let M={access_token:Le(c.id,T.sessionId,He(),c.token_version),
...
leware")}o(Mt,"rateLimit");var V=Is();process.env.NODE_ENV!=="test"&&V.use(Mt({windowMs:6e4,max:30}));V.post("/register",Ct);V.post("/login",$t);V.post("/refresh",Lt);V.post("/logout",Pt);V.get("/me",E,Ut);var qt=V;import{Router as Qs}from"npm:express@4.21.2";import{z as W}from"npm:zod@3.24.1";function g(e){let t=e.userId;if(typeof t!="string"||t.length===0)throw new m("AUTHENTICATION_REQUIRED","Authentication is required.");return t}o(g,"getUserId");function A(...e){return Object.freeze(Object.fromEntries(e.map(t=>[t,t])))}o(A,"asEnum");var Ve=A("NORTHERN","SOUTHERN","EQUATORIAL"),w=A("SPRING","SUMMER","AUTUMN","WINTER","YEAR_ROUND"),ta=A("METRIC",
...
DARD_POTTING","PEAT_BASED","COCO_COIR","SEMI_HYDRO_LECA","OTHER"),ce=A("INDOOR","OUTDOOR"),ee=A("NONE","HEATED_DRY_WINTER","AIR_CONDITIONED","HUMID_ROOM"),na=A("THRIVING","NEEDS_ATTENTION","CRITICAL",
"DORMANT"),ra=A("WALK","RUN","CYCLE","SWIM","STRENGTH","YOGA","HIIT","SPORT","OTHER"),sa=A("LOW","MODERATE","VIGOROUS"),oa=A("HEAVIEST_WEIGHT","BEST_ESTIMATED_1RM","BEST_REP_COUNT"),ia=A("BREAKFAST","\
LUNCH","DINNER","SNACK"),de=A("MALE","FEMALE","PREFER_NOT_TO_SAY"),Y=A("SEDENTARY","LIGHTLY_ACTIVE","MODERATELY_ACTIVE","VERY_ACTIVE","EXTRA_ACTIVE"),aa=A("LOSE","MAINTAIN","GAIN"),ua=A("GRAM","MILLIL\
ITRE","PIECE","CUP","TABLESPOON","SLICE","CUSTOM"),la=A("SYNCED","PENDING","SYNCING","FAILED"),ca=A("PENDING","SENT","DELIVERED","FAILED","SUPPRESSED","CANCELLED");function We(e){if(!Number.isFinite(e))throw new RangeError(`roundHalfUp expected a finite number, received ${e}`);return Math.sign(e)*Math.floor(Math.abs(e)+.5)}o(We,"roundHalfUp");function K(e,t){if(!Number.
...
e,t),r)}o(Ft,"clamp");var Ds=[w.WINTER,w.WINTER,w.SPRING,w.SPRING,w.SPRING,w.SUMMER,w.SUMMER,w.SUMMER,w.AUTUMN,w.AUTUMN,w.AUTUMN,w.WINTER],Ns=Object.freeze({[w.WINTER]:w.SUMMER,[w.SUMMER]:w.WINTER,[w.SPRING]:w.AUTUMN,[w.AUTUMN]:w.
SPRING,[w.YEAR_ROUND]:w.YEAR_ROUND});function vs(e,t){if(t===Ve.EQUATORIAL)return w.YEAR_ROUND;let r=Ds[e-1];if(r===void 0)throw new RangeError(`seasonForMonth expected a month in 1..12, received ${e}`);
return t===Ve.NORTHERN?r:Ns[r]}o(vs,"seasonForMonth");function Ht(e,t){let r=/^(\d{4})-(\d{2})-(\d{2})$/.exec(e);if(!r?.[2])throw new RangeError(`seasonForLocalDate expected a YYYY-MM-DD date, receive\
d "${e}"`);let n=Number(r[2]);if(n<1||n>12)throw new RangeError(`seasonForLocalDate received an out-of-range month in "${e}"`);return vs(n,t)}o(Ht,"seasonForLocalDate");var Ss=Object.freeze({[w.SPRING]:.95,[w.SUMMER]:.8,[w.AUTUMN]:1.15,[w.WINTER]:1.4,[w.YEAR_ROUND]:1}),Os=Object.freeze({[Z.LOW]:1.25,[Z.MEDIUM]:1.1,[Z.BRIGHT_INDIRECT]:1,[Z.DIRECT_SUN]:.85}),Cs=Object.
freeze({[$.FABRIC]:.75,[$.TERRACOTTA]:.8,[$.CONCRETE]:.9,[$.CERAMIC_GLAZED]:1,[$.OTHER]:1,[$.METAL]:1.05,[$.PLASTIC]:1.1}),$s=Object.freeze({[O.ORCHID_BARK]:.75,[O.CACTUS_SUCCULENT]:.85,[O.GARDEN_SOIL]:.95,
[O.STANDARD_POTTING]:1,[O.OTHER]:1,[O.PEAT_BASED]:1.1,[O.COCO_COIR]:1.1,[O.SEMI_HYDRO_LECA]:1.3}),Ls=Object.freeze({[ce.INDOOR]:1,[ce.OUTDOOR]:.85}),Ps=Object.freeze({[ee.HEATED_DRY_WINTER]:.85,[ee.AIR_CONDITIONED]:.9,
[ee.NONE]:1,[ee.HUMID_ROOM]:1.2});function Us(e){if(e==null)return 1;if(!Number.isFinite(e)||e<=0)throw new RangeError(`potDiameterFactor expected a positive diameter, received ${e}`);return e<10?.8:e<
15?.9:e<20?1:e<30?1.15:e<40?1.3:1.45}o(Us,"potDiameterFactor");function Ms(e){return e===!1?1.15:1}o(Ms,"drainageFactor");function Vt(e){let{baseIntervalDays:t,minIntervalDays:r,maxIntervalDays:n,season:s,
...
exceeds max ${n}`);let u=Ss[s],l=Os[a],c=e.potMaterial?Cs[e.potMaterial]:1,d=Us(e.potDiameterCm),f=Ms(e.hasDrainage),y=c*d*f,b=Ls[i],T=e.soilType?$s[e.soilType]:1,D=i===ce.OUTDOOR?1:e.indoorClimate?Ps[e.
...
fDiameter:d,fDrainage:f,fEnv:M,fPlacement:b,fSoil:T,fClimate:D,rawInterval:Se,effectiveIntervalDays:ls,clamped:Oe}}o(Vt,"computeWateringInterval");var ka=Object.freeze({protein:4,carbohydrate:4,fat:9});var ba=Object.freeze({[de.MALE]:5,[de.FEMALE]:-161,[de.PREFER_NOT_TO_SAY]:-78}),xa=Object.freeze({[Y.SEDENTARY]:1.2,[Y.LIGHTLY_ACTIVE]:1.375,[Y.MODERATELY_ACTIVE]:1.55,[Y.VERY_ACTIVE]:1.725,[Y.EXTRA_ACTIVE]:1.9}),
Ia=Object.freeze({bodyMassKg:{min:30,max:400},heightCm:{min:100,max:250},ageYears:{min:16,max:120}});function _e(e,t,r){if(!Number.isFinite(e)||e<1||e>23)throw new RangeError(`metValue must be between 1.0 and 23.0, received ${e}`);if(!Number.isFinite(t)||t<=0)throw new RangeError(`bodyMassKg must be \
...
be non-negative, received ${t}`);return K(e*t,1)}o(ge,"setVolumeKg");function fe(e){let t=e.reduce((r,n)=>r+n.reps*n.weightKg,0);return K(t,1)}o(fe,"totalVolumeKg");var Ge=/^\d{4}-\d{2}-\d{2}$/;function qs(e,t){if(!Ge.test(e)||!Ge.test(t))throw new RangeError("local dates must be YYYY-MM-DD");return Math.round((Date.parse(t)-Date.parse(e))/864e5)}o(qs,"localDateD\
...
let r=qs(e.lastCountedDate,t);if(r<=0)return e;if(r===1){let s=e.currentLength+1;return{...e,currentLength:s,longestLength:Math.max(e.longestLength,s),lastCountedDate:t}}let n=r-1;if(n<=e.freezeTokens){
...
1),lastCountedDate:t}}o(Gt,"advanceStreakOnLog");k();function Fs(e){return{currentLength:e?.current_length??0,longestLength:e?.longest_length??0,lastCountedDate:e?.last_counted_date??null,freezeTokens:e?.freeze_tokens??0}}o(Fs,"toState");async function jt(e,t,r,n){
await e.query(`insert into streaks (user_id, streak_type)
     ...
     for update`,[t,r]),a=Gt(Fs(s[0]),n);return await e.query(`update streaks
...
streak_type = 'NUTRITION'",overall_streak:"select coalesce(max(current_length), 0)::int as v from streaks where user_id = $1 and streak_type = 'OVERALL'",all_modules_in_day:`select case when exists (
                       ...
                       ) then 1 else 0 end as v`};function Hs(e,t){return e.filter(r=>{let n=t.get(r.metric);return n!==void 0&&n>=r.gte})}o(Hs,"evaluateUnlocks");async function Yt(e,t){let{rows:r}=await e.
...
let s=new Map;for(let i of new Set(n.map(u=>u.metric))){let u=Bt[i];if(!u)continue;let{rows:l}=await e.query(u,[t]);s.set(i,Number(l[0]?.v??0))}let a=Hs(n,s);for(let i of a)await e.query(`insert into \
user_achievements (user_id, achievement_id, unlocked_at, progress_pct)
       values ($1, $2, now(), 100)
       on conflict (user_id, achievement_id)
       do update set unlocked_at = coalesce(user_achievements.unlocked_at, now()), progress_pct = 100`,[t,i.id]);return a.map(i=>i.code)}o(Yt,"evaluateAchievements");var Vs={PLANT_CARE:`select (not ex\
ists (
              ...
              from meals
              where user_id = $1 and local_date_str = $2 and deleted_at is null`};async function Ws(e,t,r){return R(async n=>{let{rows:s}=await n.query(Vs[t],[e,r]),a=s[0]?.met===!0;if(a){await jt(n,e,
t,r);let{rows:u}=await n.query(`select streak_type, last_counted_date from streaks
         ...
         for update`,[e]);u.length===3&&u.every(c=>c.last_counted_date===r)&&await jt(n,e,"OVERALL",r)}let i=await Yt(n,e);return{met:a,unlocked:i}})}o(Ws,"recordDailyLog");async function ye(e){try{let t=await R(
...
async function L(e,t,r){try{let{met:n,unlocked:s}=await Ws(e,t,r);s.length>0&&h.info({userId:e,scope:t,met:n,unlocked:s},"achievements unlocked")}catch(n){h.warn({err:n,userId:e,scope:t},"engagement u\
pdate failed (log write unaffected)")}}o(L,"recordDailyLogSafe");k();async function zt(e){let t=_(),{rows:r}=await t.query(`select p.id as plant_id, p.user_id, p.nickname, p.next_water_due_at
     ...
     where p.deleted_at is null
       ...
       do nothing`,[n.user_id,n.reminder_type,n.target_entity_id,n.target_entity_type,n.title,n.body,n.due_at_utc]);r+=s.rowCount??0}return r}o(Qt,"insertReminders");var Gs=["OFF","WINDOW","SCHEDULED_\
...
query(`select user_id, timezone, quiet_hours_mode, quiet_start_time, quiet_end_time,
     ...
     from user_settings
     where user_id = any ($1::uuid[])`,[e]);for(let s of n)t.set(s.user_id,{timezone:s.timezone,quiet_hours_mode:Gs.includes(s.quiet_hours_mode)?s.quiet_hours_mode:"OFF",quiet_start_time:Kt(s.quiet_start_time),
...
null,t.base_interval_days,t.min_interval_days,t.max_interval_days,t.photo_url??null]);return n}o(un,"createPlant");var js=new Set(["nickname","species_id","room","acquisition_date","light_exposure","p\
...
([l,c])=>c!==void 0&&js.has(l));if(n.length===0)return he(e,t);let s=n.map(([l],c)=>`${l}=$${c+3}`).join(", "),a=n.map(([,l])=>l),i=_(),{rows:u}=await i.query(`UPDATE plants SET ${s}, updated_at=now()\
...
_name, base_interval_days, min_interval_days,
            max_interval_days, default_light, default_soil, care_notes, image_url
     FROM species WHERE NOT is_custom ORDER BY common_name LIMIT 200`);return r}o(fn,"listSpecies");var yn=["WATER","FERTILIZE","PRUNE","REPOT","MIST","ROTATE","TREAT"],Bs=W.string().trim().max(2048).url().refine(e=>/^https?:\/\//i.test(e),"photo_url must be an http(s) URL"),Ys=W.object({photo_url:Bs,
...
strict(),Ks=20;function zs(e){return e.issues.slice(0,Ks).map(t=>({field:t.path.join(".")||"(root)",issue:t.message}))}o(zs,"detailsFor");function Re(e,t){let r=W.string().uuid().safeParse(e);if(!r.success)
...
try{let n=g(e),s=Re(e.params.id,"id"),a=Ys.safeParse(e.body);if(!a.success)throw x("The request failed validation.",zs(a.error));let i=a.data,u=await mn(n,s,{photo_url:i.photo_url,photo_storage_key:i.
...
r(n)}}o(In,"removeGrowthEntry");async function Dn(e,t,r){try{let n=await fn(e.query.q);t.json(n)}catch(n){r(n)}}o(Dn,"searchSpecies");var I=Qs();I.use(E);I.get("/species",Dn);I.get("/",wn);I.post("/",En);I.get("/:id",hn);I.put("/:id",Rn);I.delete("/:id",An);I.post("/:id/care",Tn);I.get("/:id/care",kn);I.post("/:id/growth",bn);I.get(
"/:id/growth",xn);I.delete("/:id/growth/:entryId",In);var Nn=I;import{Router as Js}from"npm:express@4.21.2";k();async function vn(e){if(e.length===0)return new Map;let t=_(),{rows:r}=await t.query(`select workout_id, id, set_index, reps,
...
getWorkoutHandler");var Xs=new Set(["WALK","RUN","CYCLE","SWIM","STRENGTH","YOGA","HIIT","SPORT","OTHER"]);async function Mn(e,t,r){try{let n=e.body;if(!n.activity_type||!Xs.has(n.activity_type))throw x(
...
n);t.json({exercises:s})}catch(n){r(n)}}o(Fn,"searchExercises");async function Hn(e,t,r){try{let n=await Ln(z(e));t.json({personal_records:n})}catch(n){r(n)}}o(Hn,"getPersonalRecordsHandler");var P=Js();P.use(E);P.get("/exercises",Fn);P.get("/personal-records",Hn);P.get("/summary",qn);P.get("/",Pn);P.post("/",Mn);P.get("/:id",Un);var Vn=P;import{Router as ro}from"npm:express@4.21.2";import{z as v}from"npm:zod@3.24.1";k();var Gn=`id, name, brand,
...
query(`update foods
      ...
      where id = $1 and created_by = $2 and is_custom and deleted_at is null`,[e,t]);return(n??0)>0}o(Yn,"softDeleteCustomFood");var Zs=2e3;async function Kn(e,t){let r=_(),{rows:n}=await r.query(`sel\
ect id, meal_type,
...
carbs_g:0,fat_g:0});return{meals:s,water_ml_total:Number(a?.water_ml_total??0),water_goal_ml:a?.water_goal_ml??Zs,totals:i}}o(Kn,"getDailySummary");async function Te(e,t){return R(async r=>{let n=t.items.
...
ogWater");var zn=["BREAKFAST","LUNCH","DINNER","SNACK"],Ye=["GRAM","MILLILITRE","PIECE","CUP","TABLESPOON","SLICE","CUSTOM"];function eo(){return new Date().toISOString().slice(0,10)}o(eo,"todayUtcDateStr");function Ke(e){
return typeof e=="string"&&/^\d{4}-\d{2}-\d{2}$/.test(e)}o(Ke,"isValidDateStr");function to(e,t){let r=v.string().uuid().safeParse(e);if(!r.success)throw new m("VALIDATION_FAILED",`${t} must be a UUID\
.`,{details:[{field:t,issue:"invalid"}]});return r.data}o(to,"requireUuidParam");async function Qn(e,t,r){try{let n=e.query.q;if(n!==void 0&&typeof n!="string")throw new m("VALIDATION_FAILED","Query p\
arameter q must be a string.");let s=g(e),a=await jn((n??"").trim(),s);t.status(200).json({foods:a})}catch(n){r(n)}}o(Qn,"searchFoodsHandler");var no=v.object({name:v.string().trim().min(1).max(120),brand:v.
...
optional()}).strict();async function Xn(e,t,r){try{let n=g(e),s=no.safeParse(e.body);if(!s.success)throw new m("VALIDATION_FAILED","The request failed validation.",{details:s.error.issues.slice(0,20).
...
deleted:i.deleted,retention_days:te}]});t.status(201).json(i.food)}catch(n){r(n)}}o(Xn,"createCustomFoodHandler");async function Jn(e,t,r){try{let n=g(e),s=to(e.params.id,"id");if(!await Yn(s,n))throw N();
t.json({status:"deleted"})}catch(n){r(n)}}o(Jn,"deleteCustomFoodHandler");async function Zn(e,t,r){try{let n=e.query.date??eo();if(!Ke(n))throw new m("VALIDATION_FAILED","date must be YYYY-MM-DD.");let s=g(
...
void 0});await ye(a),t.status(201).json(i)}catch(n){r(n)}}o(tr,"logWaterHandler");var U=ro();U.use(E);U.get("/foods/search",Qn);U.post("/foods",Xn);U.delete("/foods/:id",Jn);U.get("/summary",Zn);U.post("/meals",er);U.post("/water",tr);var nr=U;import{Router as io}from"npm:express@4.21.2";k();var so=1e4,rr=2e3;async function sr(e,t){let r=_(),[n,s,a,i,u]=await Promise.all([r.query(`select current_length, longest_length
         from streaks
...
overdue:Number(a.rows[0]?.count??0)},fitness:{steps:c,goal:so},nutrition:{calories_consumed:d,target:rr},today_list:f}}o(sr,"getDashboard");function oo(){return new Date().toISOString().slice(0,10)}o(oo,"todayUtcDateStr");async function or(e,t,r){try{let n=typeof e.query.date=="string"&&/^\d{4}-\d{2}-\d{2}$/.test(e.query.date)?e.query.date:
oo(),s=g(e),a=await sr(s,n);t.status(200).json(a)}catch(n){r(n)}}o(or,"getDashboardHandler");var ze=io();ze.use(E);ze.get("/",or);var ir=ze;import{Router as ao}from"npm:express@4.21.2";k();async function ar(e){let t=_(),{rows:r}=await t.query(`select a.id as a_id, a.code, a.name, a.description, a.module, a.icon,
...
r(n)}});var pr=be;import{Router as co}from"npm:express@4.21.2";import{z as G}from"npm:zod@3.24.1";k();var lo=5;async function fr(e,t){try{return await gr(e,t)}catch(r){if(typeof r=="object"&&r!==null&&r.code==="23505")return gr(e,t);throw r}}o(fr,"registerToken");async function gr(e,t){return R(async r=>{
let{rows:n}=await r.query(`select id, user_id from device_push_tokens
       ...
       where id in (
         ...
         offset $2
       )`,[e,lo]);let{rows:i}=await r.query(`select id, platform, device_label, app_version, permission_status, last_confirmed_at
...
if(e.length===0)return;await _().query(`update device_push_tokens
     ...
     where token = any ($1::text[]) and status = 'ACTIVE'`,[e,t])}o(wr,"revokeTokens");var _o=G.object({expo_push_token:G.string().min(20).max(200).regex(/^Expo(nent)?PushToken\[.+\]$/),platform:G.enum(["IOS","ANDROID"]),client_installation_id:G.string().uuid(),device_label:G.string().max(
64).optional(),app_version:G.string().max(20).optional(),permission_status:G.enum(["GRANTED","DENIED","UNDETERMINED"])}).strict(),Qe=co();Qe.use(E);Qe.post("/",async(e,t,r)=>{try{let n=_o.safeParse(e.
...
g(e),{...n.data,device_label:n.data.device_label?.trim()||void 0});t.status(200).json({id:s,devices:a})}catch(n){r(n)}});var hr=Qe;import{Router as xo}from"npm:express@4.21.2";import{z as p}from"npm:zod@3.24.1";k();async function Er(e,t,r,n){let s=_(),{rows:a}=await s.query(`insert into sync_events (user_id, client_idempotency_key, entity_type, payload)
     values ($1, $2, $3, $4)
...
$2`,[t,r]);return s[0]?.id??null}o(Je,"findEntityIdByKey");var mo=50,xe=p.string().regex(/^\d{4}-\d{2}-\d{2}$/).refine(e=>{let t=Date.parse(e);return t>Date.now()-366*864e5&&t<Date.now()+2*864e5},"local_date_str outside the accepted window"),po=p.object({plant_id:p.
string().uuid(),action_type:p.enum(["WATER","FERTILIZE","PRUNE","REPOT","MIST","ROTATE","TREAT"]),note:p.string().max(500).optional(),local_date_str:xe}).strict(),go=p.object({set_index:p.number().int().
min(1).max(200).optional(),reps:p.number().int().min(0).max(1e3),weight_kg:p.number().min(0).max(1e3)}).strict(),fo=p.object({activity_type:p.string().min(1).max(40),duration_mins:p.number().int().min(
...
steps:p.number().int().min(0).max(2e5).optional(),note:p.string().max(500).optional(),local_date_str:xe,sets:p.array(go).max(200).optional()}).strict(),yo=p.object({food_id:p.string().uuid().optional(),
...
number().min(0).max(1e4),carbs_g:p.number().min(0).max(1e4),fat_g:p.number().min(0).max(1e4)}).strict(),wo=p.object({meal_type:p.enum(["BREAKFAST","LUNCH","DINNER","SNACK"]),note:p.string().max(500).optional(),
local_date_str:xe,items:p.array(yo).min(1).max(50)}).strict(),ho=p.object({amount_ml:p.number().int().min(1).max(5e3),goal_ml_at_log:p.number().int().min(1).max(2e4).optional(),local_date_str:xe}).strict(),
Eo=p.object({client_idempotency_key:p.string().regex(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i),entity_type:p.enum(["PLANT_CARE_EVENT","WORKOUT","MEAL","WATER_LOG"]),payload:p.
unknown()}).strict(),Ro=p.object({events:p.array(Eo).min(1).max(mo)}).strict();function Ao(e){return typeof e=="object"&&e!==null&&e.code==="23505"}o(Ao,"isUniqueViolation");var To={PLANT_CARE_EVENT:"\
plant_care_events",WORKOUT:"workouts",MEAL:"meals",WATER_LOG:"water_logs"},ko={PLANT_CARE_EVENT:"PLANT_CARE",WORKOUT:"FITNESS",MEAL:"NUTRITION",WATER_LOG:null};async function bo(e,t,r,n){switch(r){case"\
PLANT_CARE_EVENT":{let s=po.parse(n);return await Ee(e,s.plant_id,s.action_type,s.note,s.local_date_str,t),Je("plant_care_events",e,t)}case"WORKOUT":{let s=fo.parse(n),a=(s.sets??[]).map((c,d)=>{let f=ge(
...
total_volume_kg:i,steps:s.steps,note:s.note,local_date_str:s.local_date_str,client_idempotency_key:t,sets:a.length>0?a:void 0})).id}case"MEAL":{let s=wo.parse(n);return(await Te(e,{...s,client_idempotency_key:t})).
id}case"WATER_LOG":{let s=ho.parse(n);return(await ke(e,{...s,client_idempotency_key:t})).id}}}o(bo,"applyEvent");async function Ar(e,t,r){try{let n=g(e),s=Ro.safeParse(e.body);if(!s.success)throw new m(
...
result_entity_id,error_code:l.error_code});continue}try{let d=await bo(n,u,i.entity_type,i.payload);await Xe(l.id,d);let f=ko[i.entity_type],y=i.payload.local_date_str;f&&y?await L(n,f,y):i.entity_type===
"WATER_LOG"&&await ye(n),a.push({client_idempotency_key:u,status:"PROCESSED",replay:!1,entity_id:d,error_code:null})}catch(d){if(Ao(d)){let D=await Je(To[i.entity_type],n,u);if(D){await Xe(l.id,D),a.push(
...
FAILED",replay:!1,entity_id:null,error_code:b})}}t.status(200).json({results:a})}catch(n){r(n)}}o(Ar,"drainOutboxHandler");var Ze=xo();Ze.use(E);Ze.post("/outbox",Ar);var Tr=Ze;import{Router as Co}from"npm:express@4.21.2";import{z as Do}from"npm:zod@3.24.1";k();var br=`timezone, hemisphere, locale, unit_system, theme, week_start_day,
...
_settings (user_id) values ($1) on conflict (user_id) do nothing",[e]);let{rows:r}=await t.query(`select ${br} from user_settings where user_id = $1`,[e]);return xr(r[0])}o(Ie,"getSettings");var Io=new Set(
...
cation_cap","reduce_motion","larger_text","high_contrast","analytics_opt_in"]);async function Ir(e,t){let r=Object.entries(t).filter(([u,l])=>l!==void 0&&Io.has(u));if(r.length===0)return Ie(e);let n=_();
...
user_settings set ${s}, updated_at=now()
     where user_id=$1
     returning ${br}`,[e,...a]);return xr(i[0])}o(Ir,"updateSettings");var No={hemisphere:["NORTHERN","SOUTHERN","EQUATORIAL"],unit_system:["METRIC","IMPERIAL"],theme:["LIGHT","DARK","SYSTEM"],week_start_day:["SUNDAY","MONDAY"],quiet_hours_mode:["OFF","WINDOW","SCHEDULED\
_ONLY"]},vo=["plant_care_enabled","fitness_enabled","nutrition_enabled","reduce_motion","larger_text","high_contrast","analytics_opt_in"],So=Do.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).nullable(),Dr=[
"quiet_start_time","quiet_end_time"],Oo=["quiet_hours_mode",...Dr];async function Nr(e,t,r){try{t.json(await Ie(g(e)))}catch(n){r(n)}}o(Nr,"getSettingsHandler");async function vr(e,t,r){try{let n=e.body??
{},s=[],a={};for(let[c,d]of Object.entries(No)){let f=n[c];f!==void 0&&(typeof f!="string"||!d.includes(f)?s.push({field:c,issue:`must_be_one_of:${d.join(",")}`}):a[c]=f)}for(let c of vo){let d=n[c];d!==
void 0&&(typeof d!="boolean"?s.push({field:c,issue:"must_be_boolean"}):a[c]=d)}for(let c of Dr){let d=n[c];if(d===void 0)continue;let f=So.safeParse(d);f.success?a[c]=f.data:s.push({field:c,issue:"mus\
...
led.",{details:[{field:"modules",issue:"at_least_one_module_required"}]});if(Oo.some(c=>c in a)&&l.quiet_hours_mode==="WINDOW"){if(l.quiet_start_time===null||l.quiet_end_time===null)throw new m("VALID\
...
ATION_FAILED","Quiet hours need a different start and end time.",{details:[{field:"quiet_end_time",issue:"window_start_equals_end"}]})}t.json(await Ir(i,a))}catch(n){r(n)}}o(vr,"updateSettingsHandler");var De=Co();De.use(E);De.get("/",Nr);De.put("/",vr);var Sr=De;import{Router as Mo}from"npm:express@4.21.2";import{z as et}from"npm:zod@3.24.1";k();var $o=30,Or="PENDING_DELETION",re="status, deletion_requested_at, purge_after";async function Cr(e){let t=_(),{rows:r}=await t.query(`select ${re} from users where id = $1`,[e]);return r[0]??null}o(Cr,
...
already_pending",state:n};let{rows:[s]}=await r.query(`update users
       ...
       where id = $1
       returning ${re}`,[e,$o]);return await r.query(`update auth_sessions
 ...
 from users where id = $1 for update`,[e]);if(!r)return{kind:"missing"};if(r.status!==Or)return{kind:"not_pending",state:r};let{rows:[n]}=await t.query(`update users
       ...
       returning ${re}`,[e]);return{kind:"cancelled",state:n}})}o(Lr,"cancelDeletion");var Lo=et.object({password:et.string().min(1,"Your password is required to confirm deletion.")}).strict();function Po(e){return new m("VALIDATION_FAILED","The request failed validation.",{details:e.issues.
slice(0,10).map(t=>({field:t.path.join(".")||"(root)",issue:t.message}))})}o(Po,"validationError");function Uo(e){let t=e.sessionId,r=et.string().uuid().safeParse(t);return r.success?r.data:null}o(Uo,
...
json(tt(n))}catch(n){r(n)}}o(Pr,"getAccountHandler");async function Ur(e,t,r){try{let n=Lo.safeParse(e.body??{});if(!n.success)throw Po(n.error);let s=g(e),a=await Et(s);if(!a)throw Ne();if(a.password_hash===
...
"INVALID_CREDENTIALS","That password is not right.");let i=await $r(s,Uo(e));if(i.kind==="missing")throw Ne();t.status(200).json({...tt(i.state),already_pending:i.kind==="already_pending"})}catch(n){r(
...
r deletion.");t.status(200).json(tt(n.state))}catch(n){r(n)}}o(Mr,"cancelDeletionHandler");var se=Mo();se.use(E);se.get("/",Pr);se.post("/deletion",Ur);se.delete("/deletion",Mr);var qr=se;import{ZodError as Vo}from"npm:zod@3.24.1";import{randomUUID as qo}from"node:crypto";var Fr="x-request-id",Fo=64,Ho=/^[A-Za-z0-9._-]+$/,Hr=o((e,t,r)=>{let n=e.header(Fr),a=(n&&n.length<=Fo&&Ho.test(n)?n:void 0)??qo();e.requestId=a,t.setHeader(Fr,a),r()},"requestId");function Vr(e){return e.
requestId??"unknown"}o(Vr,"getRequestId");var Wo=50;function Go(e){return e.errors.slice(0,Wo).map(t=>({field:t.path.join(".")||"(root)",issue:t.code,message:t.message}))}o(Go,"detailsFromZod");var Wr=o((e,t,r)=>{r(new m("NOT_FOUND",`No route\
 matches ${e.method} ${e.path}`))},"notFoundHandler");function jo(e){if(!(e instanceof Error))return!1;let t=e;return t.__appError===!0&&typeof t.code=="string"&&t.code in oe}o(jo,"isMarkedAppError");
var Gr=o((e,t,r,n)=>{let s=Vr(t),a=new Date().toISOString(),i;e instanceof m?i=e:e instanceof Vo?i=new m("VALIDATION_FAILED","The request failed validation.",{details:Go(e)}):e instanceof SyntaxError&&
"body"in e?i=new m("MALFORMED_REQUEST","The request body is not valid JSON."):jo(e)?i=new m(e.code,e.message):i=new m("INTERNAL_ERROR","An unexpected error occurred.",{cause:e});let u={requestId:s,code:i.
...
/`)&&(n.url=n.url.slice(r.length),n.originalUrl=n.url),a()}),t.use(Hr),t.use(Ko()),t.use(Bo({origin:e.corsOrigins,credentials:!0,exposedHeaders:["x-request-id"]})),t.use(jr.json({limit:e.bodyLimit??"1\
mb"})),t.use(Yo()),t.use("/api/auth",qt),t.use("/api/v1/plants",Nn),t.use("/api/v1/fitness",Vn),t.use("/api/v1/nutrition",nr),t.use("/api/v1/dashboard",ir),t.use("/api/v1/achievements",mr),t.use("/api\
...
uptime())})}),t.get("/api/v1",(n,s)=>{s.json({name:"PlantPal+ API",version:"v1"})}),t.use(Wr),t.use(Gr),t}o(Br,"createApp");k();import Zc from"npm:node-cron@4.6.0";k();import{createHmac as zo}from"node:crypto";var nt=100,Yr=Object.freeze([{table:"profiles",column:"user_id"},{table:"user_settings",column:"user_id"},{table:"auth_sessions",column:"user_id"},{table:"auth_tokens",column:"user_id"},{table:"email_\
...
{table:"sync_events",column:"user_id"}]);async function Kr(e=nt){let{rows:t}=await _().query(`select id, email_normalised
      ...
      limit $1`,[e]);return t}o(Kr,"findAccountsDueForPurge");function Qo(e,t){return zo("sha256",t).update(e).digest("hex")}o(Qo,"subjectHash");async function zr(e,t){return R(async r=>{let{rows:[n]}=await r.
query(`select id
         from users
        where id = $1
...
elect ${s}`,[e.id]),i={};Yr.forEach(({table:d},f)=>{i[d]=a?.[`t${f}`]??0});let u=Qo(e.id,t),{rowCount:l}=await r.query(`update audit_events
          set user_id = null,
...
c??0,await r.query("delete from users where id = $1",[e.id]),i.users=1,await r.query(`insert into audit_events (user_id, event_type, payload)
       values (null, 'ACCOUNT_ERASED', $1::jsonb)`,[JSON.stringify({subject:u,rows:i,erased_at:new Date().toISOString()})]),{erased:!0,counts:i}})}o(zr,"purgeAccount");function Xo(){let e=F();return e.AUDIT_PEPPER??e.JWT_ACCESS_SECRET}o(Xo,"pepper");async function Qr(e=nt){let t=await Kr(e),r={due:t.length,erased:0,skipped:0,failed:0,counts:{}};if(t.length===0)return r;
let n=Xo();for(let s of t)try{let a=await zr(s,n);if(!a.erased){r.skipped++;continue}r.erased++;for(let[i,u]of Object.entries(a.counts))r.counts[i]=(r.counts[i]??0)+u}catch(a){r.failed++,h.error({err:a},
"account erasure failed; will retry on the next sweep")}return h.info(r,"account erasure sweep complete"),r}o(Qr,"runPurgePass");import dd from"npm:node-cron@4.6.0";var Jo="https://exp.host/--/api/v2/push/send",Zo=100;function ei(e,t=Zo){let r=[];for(let n=0;n<e.length;n+=t)r.push(e.slice(n,n+t));return r}o(ei,"chunkMessages");async function Xr(e){let t={delivered:[],
notRegistered:[],failed:[]};for(let r of ei(e))try{let n=await fetch(Jo,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify(r)});if(!n.ok){t.failed.
...
e:s.next_water_due_at}))}o(ts,"planWateringReminders");var ti=5,ns={timezone:"UTC",quiet_hours_mode:"WINDOW",quiet_start_time:null,quiet_end_time:null,daily_notification_cap:12},Jr={hourCycle:"h23",year:"\
numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"},Zr=new Map;function ni(e){let t=Zr.get(e);if(t)return t;let r;try{r=new Intl.DateTimeFormat("en-US",{...Jr,timeZone:e})}catch{r=
new Intl.DateTimeFormat("en-US",{...Jr,timeZone:"UTC"})}return Zr.set(e,r),r}o(ni,"formatterFor");function rt(e,t){let r=ni(t).formatToParts(e),n=o(a=>r.find(i=>i.type===a)?.value??"00","part"),s=Number(
n("hour"))%24;return{dateKey:`${n("year")}-${n("month")}-${n("day")}`,minutes:s*60+Number(n("minute"))}}o(rt,"localClock");var ri=1440;function es(e){if(e===null)return null;let t=/^(\d{1,2}):(\d{2})(?::\d{2}(?:\.\d+)?)?$/.
exec(e.trim());if(!t)return null;let r=Number(t[1])*60+Number(t[2]);return r>=0&&r<ri?r:null}o(es,"wallClockMinutes");function si(e,t){if(t.quiet_hours_mode==="OFF")return!1;if(t.quiet_hours_mode==="S\
CHEDULED_ONLY")return!0;let r=es(t.quiet_start_time),n=es(t.quiet_end_time);if(r===null||n===null||r===n)return!1;let s=rt(e,t.timezone).minutes;return r<n?s>=r&&s<n:s>=r||s<n}o(si,"isWithinQuietHours");
function oi(e,t,r){let n=rt(t,r).dateKey,s=0;for(let a of e)rt(a,r).dateKey===n&&(s+=1);return s}o(oi,"sentOnLocalDay");function ii(e){let t=e.daily_notification_cap;return!Number.isFinite(t)||t<1?ns.
daily_notification_cap:Math.floor(t)}o(ii,"capOf");function rs(e,t,r={}){let n={send:[],fail:[],defer:[]},s=t.filter(i=>i.due_at_utc.getTime()<=e.getTime()).sort((i,u)=>{let l=i.due_at_utc.getTime()-u.
due_at_utc.getTime();return l!==0?l:i.id<u.id?-1:i.id>u.id?1:0}),a=new Map;for(let i of s){if(i.attempts>=ti){n.fail.push(i.id);continue}let u=r.settings?.get(i.user_id)??ns;if(si(e,u)){n.defer.push({
id:i.id,reason:"QUIET_HOURS"});continue}let l=a.get(i.user_id);if(l===void 0){let c=r.sentAt?.get(i.user_id)??[];l=Math.max(0,ii(u)-oi(c,e,u.timezone))}if(l===0){a.set(i.user_id,0),n.defer.push({id:i.
id,reason:"DAILY_CAP_REACHED"});continue}a.set(i.user_id,l-1),n.send.push(i.id)}return n}o(rs,"tick");var ss=24;async function ai(e){if(e.length===0)return 0;let t=await yr([...new Set(e.map(i=>i.user_id))]);if(t.size===0)return 0;let r=[],n=new Map;for(let i of e)for(let u of t.get(i.user_id)??[]){r.
...
RED");let a=new Set;for(let i of s.delivered)for(let u of n.get(i)??[])a.add(u);return await en([...a]),a.size}o(ai,"deliverByPush");async function os(e=new Date){let t=await zt(ss),r=ts(e,t,ss),n=await Qt(
r),s=await Zt(),a=[...new Set(s.map(f=>f.user_id))],[i,u]=await Promise.all([Xt(a),Jt(a,e)]),l=rs(e,s,{settings:i,sentAt:u});await tn(l.send),await nn(l.fail);let c=new Set(l.send),d=await ai(s.filter(
...
"No platform secret to derive from: set JWT_ACCESS_SECRET on the function explicitly.");return ui("sha256",t).update(`plantpal:${e}`).digest("hex")}o(st,"derivedSecret");function Q(e,t){let r=Deno.env.
...
Deno.env.get("SUPABASE_URL")??"https://localhost").origin,us=Ce({...li.env,NODE_ENV:"production",DATABASE_URL:is,JWT_ACCESS_SECRET:Q("JWT_ACCESS_SECRET",st("jwt-access")),AUDIT_PEPPER:Q("AUDIT_PEPPER",
...
var di=Q("TICK_SECRET",st("internal-tick")),_i=Br({corsOrigins:us.CORS_ORIGINS,basePath:`/${ve}`}),ot=ci();ot.post(`/${ve}/internal/tick`,(e,t)=>{if(e.get("authorization")!==`Bearer ${di}`){t.status(401).
...
led"},"internal tick complete")}),t.status(202).json({status:"accepted"})});ot.use(_i);h.info({slug:ve,origin:as},"PlantPal+ API starting on Supabase Edge");ot.listen(8e3);
