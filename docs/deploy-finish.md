...
2. **New → Blueprint** → connect the `rakshit-737/PlantPal-Plus` repo. Render
   reads [render.yaml](../render.yaml) and provisions the `plantpal-api` free
   web service; `JWT_ACCESS_SECRET` is generated for you.
...
4. Wait for the first deploy; migrations and seeds run at boot. Verify:
   `https://<your-service>.onrender.com/healthz` returns JSON.
5. Back in GitHub: repo **Settings → Secrets and variables → Actions →
   Variables** → new variable `PLANTPAL_API_URL` = the Render origin (no
   trailing slash), e.g. `https://plantpal-api.onrender.com`.
6. Re-run the **Deploy web to GitHub Pages** workflow (Actions tab → run
   workflow) so the site is rebuilt with the API URL baked in. Sign-in now
   works on https://rakshit-737.github.io/PlantPal-Plus/ (Chrome/Firefox;
   Safari needs step 3 below).
...
The preview profile bakes `EXPO_PUBLIC_API_URL=https://plantpal-api.onrender.com`
